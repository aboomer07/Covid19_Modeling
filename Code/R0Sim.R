...
#   mean <- params[[i]][[1]]
#   std <- params[[i]][[2]]
#   est_r0 <- estimate_R(incid=as.numeric(actual_data$confirmed), method='parametric_si',
...
est_r0 <- est_r0[c('Mean(R)', 'Std(R)', 't_end')]
names(est_r0) <- c("R_Mean", 'R_Std', 'Days')
est_r0['lower'] <- est_r0['R_Mean'] - (1.96 * est_r0['R_Std'])
est_r0['upper'] <- est_r0['R_Mean'] + (1.96 * est_r0['R_Std'])
est_r0$Method <- 'TSI'
...
for (inter in c(1, 2, 3, 4)) {
  ...
  for (t in (3 + interval):nrow(actual_data)) {
   r0_ger[t] <- sum(as.numeric(actual_data$confirmed)[t-0:3]) / sum(as.numeric(actual_data$confirmed)[t-interval:(3 + interval)])
  }
  r0_ger <- data.frame(r0_ger)
  # bind to evaluation dataframe
  r0_ger <- cbind(actual_data, r0_ger)

  int_list[[inter]] <- r0_ger
}
...
df <- rbind(est_r0, r0_ger)

df %>% ggplot() +
  geom_line(aes(x = Days, y = R_Mean, color = Method, group = Method)) +
  geom_ribbon(aes(x=Days, ymin = lower, ymax = upper, group = Method)) +
  ylab('Estimated R_0') +
  theme_minimal() +
  labs(caption = 'Data: US Daily Covid-19 Cases, Johns Hopkins University March 2020 - February 2021', color = '') +
  ggsave(paste0(outpath, 'R0_Estimation.png'), width = 10, height = 5)
