...
gen_distribution <- function(k, mean, var, type, delta) {
	...
	if (type == 'gamma') {
		...
		omega <- omega + a * b * (2 * cdf_gamma(k - 1, a + 1, b) - 
...
samp_pois <- function(R_val, study_len, num_people, sim_mu, sim_var, sim_type) {

	data <- data.frame(rep(R_val, (study_len/length(R_val))))
	names(data) <- "Rt"
	data$sim_mu <- rep(sim_mu, study_len)
	data$sim_var <- rep(sim_var, study_len)
	...
	for (t in range) {
		...
		dist <- gen_distribution(t, data[t, ]$sim_mu, data[t, ]$sim_var, sim_type)
...
serial_ests <- function(est_mu, est_var, num_vars, dists) {
  num_vars <- 10
  params <- list()
